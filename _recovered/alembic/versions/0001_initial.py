...
survey_status = sa.Enum("draft", "published", "closed", name="survey_status")
...
collab_role = sa.Enum("owner", "editor", "viewer", name="collab_role")


def upgrade() -> None:
    survey_status.create(op.get_bind(), checkfirst=True)
    question_type.create(op.get_bind(), checkfirst=True)
    collab_role.create(op.get_bind(), checkfirst=True)
...
def downgrade() -> None:
    ...
    op.drop_table("users")
    collab_role.drop(op.get_bind(), checkfirst=True)
