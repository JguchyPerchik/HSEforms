...
def _sign_question(q_dict: dict) -> dict:
    """Walk a serialised question and replace bare media_url with a signed one."""
    if q_dict.get("media_url"):
        q_dict["media_url"] = sign_path(q_dict["media_url"])
    for opt in q_dict.get("options") or []:
        if opt.get("media_url"):
            opt["media_url"] = sign_path(opt["media_url"])
    return q_dict
...
def _public_detail(s: Survey) -> SurveyDetail:
    ...
    for q in s.questions:
        ...
        q_dict = q_out.model_dump()
        _sign_question(q_dict)
        questions.append(QuestionOut.model_validate(q_dict))
    return SurveyDetail(
        id=s.id,
        owner_id=s.owner_id,
        title=s.title,
...
async def submit_response(
    ...
    # Persist per-question variant assignments computed client-side. Keys are
    # already strings (JSON), values are ints. Validate range loosely.
    cleaned: dict[str, int] = {}
    for k, v in (payload.variant_assignments or {}).items():
        if isinstance(v, int) and v >= -1:
            cleaned[str(k)] = v
    resp.variant_assignments = cleaned
    ...
    return ResponseOut(
        ...
        variant_assignments=resp.variant_assignments or {},
    )
