...
class SurveyAnalytics(BaseModel):
    ...
    trend_bin: str = "day"
    # Время прохождения (submitted_at - started_at) только по завершённым.
    # Считаем сразу две метрики — у них разное назначение:
    #   • median — «сколько занимает типичный респондент»
    #     Устойчива к выбросам (вкладка открыта на 6 часов = одна точка
    #     в хвосте, не сдвигает 50-й перцентиль).
    #   • avg — среднее с отбрасыванием выбросов > 4 ч (вкладка забыта).
    #     Если avg сильно больше median → распределение скошено, часть
    #     респондентов залипает. Сигнал «вопрос непонятный, перепиши».
    # Поля None, если завершённых ответов ещё нет.
    median_completion_seconds: int | None = None
    avg_completion_seconds: int | None = None
